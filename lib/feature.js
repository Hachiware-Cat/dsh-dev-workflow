...
export function defaultHeader(opts) {
  ...
  return [
    ...
    // 需求确认小节默认就在模板里:`buildDocument` 拿到 `requirement` 时会整段改写它,
    // 拿不到时这一行就是"还没登记"的实话(而不是缺一节,让人以为这门不存在)。
    '', '## 需求确认', '- ⏳ 未登记(用户确认需求最终稿前,生产角色不派活)',
    '', '## 产出文件', '- (无)',
...
export function buildDocument(currentText, a) {
  ...
  /**
   * 需求确认小节:`args.requirement` 是 `requirementInfoOf()` 出来的视图对象(§需求确认门)。
   * 它进文档是刻意的 —— 批准钉的是**指纹**,而指纹只有写进文档,人才能对着它看出
   * "这份需求改过之后,上次那句确认已经不算数了"。
   */
  if (args.requirement && typeof args.requirement === 'object') {
    text = replaceSection(text, '需求确认', requirementSectionRows(args.requirement))
  }
...
export function summarize(text) {
  return {
    ...
    requirement: readSection(text, '需求确认'),
...
// ── 需求确认门:记账(②)与上锁(①)────────────────────────────────────────────
/**
 * 由来:pm 的人设写着「需求最终稿必须停下来交用户审核(唯一不受「开工授权」影响的硬关卡)」,
 * 而这条规矩此前**没有任何机制托底** —— 全仓库 grep「硬关卡 / 用户审核」只命中那句人设:
 * 门、校验、断言一个都没有。于是 pm 完全可以不等用户点头就把生产角色派下去。
 *
 * 现在合成一件事(② 记账在前,① 上锁在后,锁的钥匙就是记账那个字段):
 *   · ② 记账:`workflow_state_save` 收一个 `requirementApproval`,把"用户批准的是**哪一版**需求"
 *     记成结构化事实(文档 SHA256 指纹 + 时间 + 谁登记的 + 有没有外部证据),三处状态面同源显示;
 *   · ① 上锁:`kickoff` 与 `relay_spawn` 在"生产角色**首次**入场"这一格拦下来。
 *     判据不是"某条待办勾没勾" —— 那是自证,换个地方写而已 —— 而是 ② 那个指纹字段。
 *
 * 两条硬约束(缺一条这道门就是假的):
 *   · **钥匙必须在能问用户的那一方手里**。DSH 的 userQuestions seam 对"归属于另一个 agent 的
 *     子级"直接抛 `DELEGATED_CALLER`(源码判据:`agents.roots().includes(agent)` 不成立即拒),
 *     要求子会话"把未决问题写进最终结果"。pm 是子会话 —— 它**问不到用户**,
 *     所以它也不能替用户开门:登记只收 live runtime root(主会话)的调用。
 *   · **必须留逃生阀**。`.active` 写一行 `approval=skip` 即放行(用户侧,不经模型);
 *     配置 `requirementApproval: track|off` 可整档降级。误拦会让流程直接停摆,
 *     比"少拦一次"贵得多 —— 与本项目"建议必须能被照做"是同一个口径。
 */

/** 需求最终稿的默认落点(pm 的产出契约);档案里可用 `requirementFile` 覆盖。 */
const DEFAULT_REQUIREMENT_REL = 'docs/workflow/项目经理.md'
/**
 * 需求阶段**必须在场**的角色(不受门限制)。
 *
 * 协调者与只读角色天然豁免;此外还有 @arch:pm 的人设要求"技术可行性问题一律互呼 @arch,
 * 不自行拍定" —— 把架构师挡在门外等于让需求根本定不了稿,门把自己锁死了。
 * 其余角色(生产侧)默认受约束;档案里给角色写 `awaitRequirement: true|false` 可逐角色覆盖,
 * 内置档没有写这个字段,所以升级后 standard/lean3 里的 @be/@fe(以及 platform6 的 @dba 这类
 * 自定义生产角色)自动受门约束,不需要改档案。
 */
const REQUIREMENT_FREE_ROLES = ['arch']
/** 登记批准时,「近这么久内观测到主会话调用过 ask_user_question」才算有外部事实可核。 */
const APPROVAL_EVIDENCE_MS = 30 * 60 * 1000

/** `requirementApproval` 配置的合法档位(enforce=记账+拦;track=只记账;off=都不做)。 */
export function resolveRequirementMode(value) {
  const raw = String(value === undefined || value === null ? '' : value).trim().toLowerCase()
  if (raw === '') return { mode: 'enforce', raw, known: true }
  if (['enforce', 'on', 'true', '1', 'block'].indexOf(raw) !== -1) return { mode: 'enforce', raw, known: true }
  if (['track', 'record', 'audit', 'warn'].indexOf(raw) !== -1) return { mode: 'track', raw, known: true }
  if (['off', 'false', '0', 'disabled', 'none'].indexOf(raw) !== -1) return { mode: 'off', raw, known: true }
  // 认不出的值按 enforce(要的是门,不是静默放行),但**如实标出来** —— 打错字的配置必须看得见。
  return { mode: 'enforce', raw, known: false }
}

/** 这个角色是否受需求确认约束(纯函数;档案里的 `awaitRequirement` 优先)。 */
export function requirementGatedRole(role, coordinator) {
  if (!role || typeof role !== 'object') return false
  if (typeof role.awaitRequirement === 'boolean') return role.awaitRequirement
  const id = String(role.id || '')
  if (id === '') return false
  if (id === String(coordinator || '')) return false
  if (role.readonly === true) return false
  if (REQUIREMENT_FREE_ROLES.indexOf(id) !== -1) return false
  return true
}

/**
 * 门判定(纯函数)。六种状态:disabled / skipped / approved / drifted / doc_missing / unregistered。
 *
 * `drifted` 复用 api lint 那套"**指纹漂移即作废上次结论**"的口径:批准钉的是某一版需求文本;
 * 需求文档改过之后,那句"用户已确认"自动失效,必须重新确认 —— 否则"确认"会变成一个
 * 一次性的、之后永久有效的橡皮图章。
 */
export function requirementDecision(input) {
  const i = input || {}
  const mode = String(i.mode || 'enforce')
  const approval = i.approval && typeof i.approval === 'object' ? i.approval : null
  const out = {
    mode, state: 'unregistered', locked: false, drift: false,
    digestWas: '', digestNow: String(i.docDigest || ''), approval,
  }
  if (mode === 'off') { out.state = 'disabled'; return out }
  if (i.escape === true) { out.state = 'skipped'; return out }
  if (!approval) out.state = 'unregistered'
  else if (i.docExists !== true || !i.docDigest) { out.state = 'doc_missing'; out.digestWas = String(approval.digest || '') }
  else if (String(approval.digest || '') !== String(i.docDigest)) {
    out.state = 'drifted'
    out.drift = true
    out.digestWas = String(approval.digest || '')
    out.digestNow = String(i.docDigest)
  } else out.state = 'approved'
  out.locked = mode === 'enforce' && (out.state === 'unregistered' || out.state === 'drifted' || out.state === 'doc_missing')
  return out
}

/**
 * 需求确认的**一句话状态**。五个界面(workflow_state_status 的两个分支 / relay status /
 * relay_spawn / kickoff)与 save 回执全部走这一个函数 —— 同一件事在几处说成几样,
 * 是这个项目反复踩过的坑(见 apiCoverageText 的由来)。
 */
export function requirementStateText(info) {
  const v = info || {}
  const docPath = String(v.docPath || DEFAULT_REQUIREMENT_REL)
  const gatedRoles = Array.isArray(v.gatedRoles) ? v.gatedRoles : []
  const gated = gatedRoles.length > 0 ? gatedRoles.map((r) => `@${r}`).join('/') : '(无)'
  const shot = (h) => String(h || '').slice(0, 12)
  const ap = v.approval || null
  const modeNote = `门=${v.mode || 'enforce'}${v.rawKnown === false ? `(配置值 "${v.raw}" 未识别,按 enforce)` : ''}`
  switch (v.state) {
    case 'disabled':
      return `需求确认:记账已关(配置 requirementApproval=${v.raw || 'off'})—— 不拦也不登记`
    case 'skipped':
      return `需求确认:已放行(${ACTIVE_REL} 写了 approval=skip —— 用户侧逃生阀;门不拦,也**不冒充**"用户已确认")`
    case 'approved':
      return `需求确认:已登记(by=${ap ? (ap.by || 'user') : 'user'}${ap && ap.at ? ` @${ap.at}` : ''}${ap && ap.digest ? ` 指纹 ${shot(ap.digest)}…` : ''}`
        + ` | 文档 ${docPath}${v.docChars ? `(${v.docChars} 字)` : ''}`
        + ` | 证据=${ap && ap.evidence ? ap.evidence : '⚠️ 未观测到向用户提问,这条是自证的'})`
    case 'drifted':
      return `需求确认:**已漂移**(需求文档在登记后被改过:登记 ${shot(v.digestWas)}… → 现在 ${shot(v.digestNow)}…)`
        + ` —— 上次那句"用户已确认"已作废,${gated} 会被门拦下,请重新确认`
    case 'doc_missing':
      return `需求确认:**已登记但文档读不到**(${docPath} 不存在或读不出)—— 无从核对批准的是哪一版,按未确认处理`
    default:
      return `需求确认:**未登记**(${modeNote})—— ${gated} 首次入场${v.mode === 'track' ? '**不会被拦**(档位 track:只记账不拦)' : '会被拦下'};`
        + `出路:主会话登记(workflow_state_save requirementApproval={by:"user"})`
        + ` / 用户在 ${ACTIVE_REL} 写一行 approval=skip / 配置 requirementApproval=track|off`
  }
}

/** 「需求确认」小节的正文(纯函数,渲染进 流程状态.md)。 */
export function requirementSectionRows(info) {
  const v = info || {}
  const ap = v.approval || null
  const rows = []
  switch (v.state) {
    case 'approved':
      rows.push(`- ✅ 已登记:by=${ap ? (ap.by || 'user') : 'user'}${ap && ap.at ? ` @${ap.at}` : ''} | 登记人自述:${(ap && ap.note) ? ap.note : '(未写)'}`)
      rows.push(`- 对象:${String(v.docPath || DEFAULT_REQUIREMENT_REL)}(${v.docChars || 0} 字)指纹 ${String(ap && ap.digest || '').slice(0, 12)}… —— 该文档一改,这条自动作废(指纹漂移)`)
      rows.push(`- 证据:${(ap && ap.evidence) ? ap.evidence : '⚠️ 未观测到登记前 30 分钟内主会话调用 ask_user_question —— 这条登记是**自证**的,请人工核对'}`)
      break
    case 'drifted':
      rows.push(`- ⚠️ 已漂移:登记 ${String(v.digestWas || '').slice(0, 12)}… → 现在 ${String(v.digestNow || '').slice(0, 12)}… —— 需求文档改动过,请重新确认`)
      break
    case 'skipped':
      rows.push(`- 🔓 已放行:${ACTIVE_REL} 写了 approval=skip(用户侧逃生阀;不代表用户确认过需求)`)
      break
    case 'disabled':
      rows.push(`- (门已关:配置 requirementApproval=${v.raw || 'off'})`)
      break
    case 'doc_missing':
      rows.push(`- ⚠️ 已登记但 ${String(v.docPath || DEFAULT_REQUIREMENT_REL)} 读不到:无从核对版本,按未确认处理`)
      break
    default:
      rows.push(`- ${v.mode === 'track' ? '📝 未登记(档位 track:只记账不拦)' : '⏳ 未登记:用户确认需求最终稿之前,生产角色不派活(kickoff 会跳过它们)'}`)
  }
  return rows.join('\n')
}
...
export function apply(ctx, config = {}) {
  ...
  /**
   * 需求确认门的档位(见文件里「需求确认门」那一节)。缺省 `enforce`:
   * 门要真的存在才有意义,而误拦的三条出路(主会话登记 / `.active` 写 approval=skip /
   * 配置降级 track|off)都在回执里点名 —— 拦下来的时候**同时给出路**,不留死路。
   */
  const REQ_MODE = resolveRequirementMode(cfg.requirementApproval)
  /**
   * "这个会话问过用户吗" —— 登记批准时唯一的**外部事实**。
   *
   * 只记进程内(与 presetAutoStats 同一口径):它回答的是"本次登记有没有事实可核",
   * 落盘只会把跨重启的旧事实翻出来冒充新证据。没有它,`by:"user"` 就纯粹是自证 ——
   * 有了它,回执能分开说"已观测到本轮向用户提问"与"未观测到,这条是自证的"。
   */
  const askUserSeen = new Map()
  /**
   * 每个 agent 只判一次。两个作用:
  ...
  // ── 需求确认门:apply 侧的取数 / 判定 / 留痕 ────────────────────────────────

  /** 需求最终稿的落点(档案可覆盖)。 */
  function requirementRelOf(profile) {
    return String((profile && profile.requirementFile) || DEFAULT_REQUIREMENT_REL)
  }

  /** 读需求文档并算指纹。读不到 = 指纹空串(调用处按 doc_missing 处理,而不是"没这回事")。 */
  function requirementDocOf(profile, root) {
    const rel = requirementRelOf(profile)
    const m = readMaybe(rel, root)
    return {
      rel,
      exists: m.exists === true,
      digest: m.exists ? sha256Hex(m.text) : '',
      chars: m.exists ? String(m.text).length : 0,
    }
  }

  /**
   * 用户侧逃生阀:`.active` 里写一行 `approval=skip`(skip/off/none/bypass 都收)。
   *
   * 刻意做成**文件**而不是工具参数:钥匙在用户手里,不经过模型 —— 与 `.active` 写 `off`
   * 压过一切是同一条设计。它只放行,不会让任何回执宣称"用户已确认过需求"。
   */
  function requirementEscape(root) {
    const act = readMaybe(ACTIVE_REL, root)
    if (!act.exists || !act.text) return false
    return /(^|\s)approval\s*=\s*(skip|off|none|bypass)\b/i.test(String(act.text))
  }

  /** 当前项目的需求确认视图 —— status / save / spawn / kickoff 四处**同源**取它。 */
  function requirementInfoOf(profile, root, slot) {
    const doc = requirementDocOf(profile, root)
    const escape = requirementEscape(root)
    const d = requirementDecision({
      mode: REQ_MODE.mode,
      escape,
      approval: slot && slot.requirement,
      docExists: doc.exists,
      docDigest: doc.digest,
    })
    return Object.assign({}, d, {
      raw: REQ_MODE.raw,
      rawKnown: REQ_MODE.known,
      docPath: doc.rel,
      docExists: doc.exists,
      docChars: doc.chars,
      escape,
      gatedRoles: profile.roles.filter((r) => requirementGatedRole(r, profile.coordinator)).map((r) => r.id),
    })
  }

  /**
   * 「主会话」的判据 —— DSH 眼里**确切的 live runtime root**,与 userQuestions seam 自己的
   * 判据同源(`agents.roots().includes(agent)`;子级调用 ask_user_question 会被它抛
   * `DELEGATED_CALLER`)。查不到 `roots()` 时回落到启发式:没有父会话 **且** 不是任何项目的
   * 角色绑定(DSH 里确实存在 header 没写 parentSession 的角色子会话,所以两条都要看)。
   */
  function isRuntimeRoot(agent) {
    if (!agent) return false
    const svc = ctx.get('agents')
    if (svc && typeof svc.roots === 'function') {
      try { return svc.roots().includes(agent) === true } catch { /* 回落下面的启发式 */ }
    }
    return parentSessionOf(agent) === '' && roleSlotOfAgent(agent) === null
  }
   ...
   *
   * 不拦的四种情形,每条都有明确理由:
   *   ① 门没锁(appr 已登记且指纹未漂移 / `.active` 放了 skip / 档位是 track|off);
   *   ② 角色不受约束(协调者 / 只读 / @arch —— 见 REQUIREMENT_FREE_ROLES);
   *   ③ 该角色**已经在本项目里入场过**(仍有绑定,或台账里出现过)。这一条不是放水:
   *      全项目的补救指引都写着"relay_spawn role=X 重派即可",门若把它拦死,那些指引就成了假话;
   *   ④ relay_spawn 且调用者是**主会话**(live runtime root)→ 放行,但要求调用处**留痕**
   *      (回执一行 + 台账一笔"绕过需求确认门")。钥匙在人手里:角色子会话连用户都问不到,
   *      自然也不能替用户开门;真正能开门的只有人所在的那个会话。
   *
   * `bulk=true`(kickoff 批量开工)时第 ④ 条不成立 —— 开工这条路上,未确认就不派生产角色。
   */
  function requirementEntryDecision(profile, root, slot, roleId, opts) {
    const o = opts || {}
    const info = requirementInfoOf(profile, root, slot)
    if (!info.locked) return { blocked: false, info }
    const role = roleOf(profile, roleId)
    if (!role || !requirementGatedRole(role, profile.coordinator)) return { blocked: false, info }
    if (slot && slot.roleAgents && slot.roleAgents[roleId]) return { blocked: false, info, inFlow: 'binding' }
    const seen = (slot && Array.isArray(slot.ledger) ? slot.ledger : [])
      .some((r) => String((r && r.from) || '') === roleId || String((r && r.to) || '') === roleId)
    if (seen) return { blocked: false, info, inFlow: 'ledger' }
    if (o.bulk !== true && isRuntimeRoot(o.caller)) return { blocked: false, info, bypass: true }
    return { blocked: true, info }
  }

  /** 门的出路(拦下时的 nextActions)—— 每条都必须**照做得到**。 */
  function requirementEscapeActions(profile) {
    return [
      '主会话(人在这里的那个会话):先用 ask_user_question 把需求最终稿交给用户确认,再调 '
      + 'workflow_state_save requirementApproval={by:"user", note:"<用户原话摘要>"} 登记 —— 登记完同一条 kickoff / relay_spawn 就能派出去',
      `用户侧逃生阀(不经模型):在 ${ACTIVE_REL} 里写一行 approval=skip,门即放行(不改任何文档)`,
      '配置降级:组合里 dev-workflow 那一行加 requirementApproval: "track"(只记账不拦)或 "off"(都不做)',
      '逐角色豁免:档案里给该角色写 awaitRequirement: false(自定义生产角色默认受门约束)',
      `门的当前对象是 ${requirementRelOf(profile)}(档案可用 requirementFile 覆盖)`,
    ]
  }

  /**
   * 证据键 = **会话 + 项目根**。只用会话 id 会松一格:同一个会话在 A 项目问过用户,
   * 30 分钟内去 B 项目登记批准时会拿 A 的提问当证据。带上根之后,这条外部事实变成
   * "**这个会话为这个项目**问过用户" —— 口径与"批准钉的是这个项目的哪一版需求"对齐。
   * 根不一致时匹配不上 → 如实报"未观测到"(宁可漏报证据,不许虚报)。
   */
  function askEvidenceKey(agentId, root) {
    return `${String(agentId || '')}\u0000${normalizeRootKey(root)}`
  }

  /**
   * 登记"用户已确认需求最终稿"(② 记账)。返回登记结果,并把事实写进槽 + 台账。
   *
   * 三条规矩:
   *   · **只收主会话的登记**(live runtime root):pm 这类角色子会话连用户都问不到
   *     (`DELEGATED_CALLER`),所以它写的"用户已确认"没有任何事实可核 —— 直接拒,并给出正确路径;
   *   · **必须能钉住版本**:需求文档读不到就不登记 —— 批准一个不存在的文档,等于把"确认"变成口号;
   *   · **证据与自证分开说**:进程内观测到主会话近 30 分钟内调用过 `ask_user_question` 才记时间戳,
   *     否则如实写"未观测到"。不拦,但绝不让它冒充"有据可查"。
   */
  function registerRequirementApproval(opts) {
    const o = opts || {}
    const slot = o.slot
    const req = o.req && typeof o.req === 'object' ? o.req : {}
    const doc = requirementDocOf(o.profile, o.root)
    const base = {
      by: String(req.by || 'user'),
      note: String(req.note || '').slice(0, 200),
      docPath: doc.rel, docChars: doc.chars, digest: doc.digest,
    }
    if (o.isChild === true || !isRuntimeRoot(o.caller)) {
      return Object.assign(base, {
        op: 'rejected', reason: 'delegated_caller',
        note: '批准登记只收**主会话**(DSH 眼里确切的 live runtime root)的调用:角色子会话问不到用户'
          + '(ask_user_question 对"归属于另一个 agent 的子级"直接抛 DELEGATED_CALLER),'
          + '所以它写的"用户已确认"没有任何事实可核,本次不登记。',
        nextActions: [
          '把这件事交回主会话:由它先用 ask_user_question 向用户确认需求最终稿,再由它调 workflow_state_save requirementApproval={by:"user", note:"…"} 登记',
          `用户也可以直接放行(不经模型):在 ${ACTIVE_REL} 里写一行 approval=skip`,
        ],
      })
    }
    if (!doc.exists) {
      return Object.assign(base, {
        op: 'rejected', reason: 'doc_missing',
        note: `要确认的对象 ${doc.rel} 现在读不到 —— 批准一个不存在的文档等于把"确认"变成口号,本次不登记。`,
        nextActions: [
          `先让 ① 项目经理产出需求最终稿(relay_spawn role=${o.profile.coordinator} 或让已在场的 pm 会话写 ${doc.rel}),再登记`,
          '若这个项目的需求文档在别的路径:档案里加 requirementFile: "<相对路径>" 后重新登记',
        ],
      })
    }
    const seenAt = Number(askUserSeen.get(askEvidenceKey(o.caller && o.caller.id, o.root)) || 0)
    const fresh = seenAt > 0 && (Date.now() - seenAt) <= APPROVAL_EVIDENCE_MS
    slot.requirement = {
      at: makeTS(), by: base.by, note: base.note,
      digest: doc.digest, docPath: doc.rel, docChars: doc.chars,
      evidence: fresh ? `ask_user_question@${makeTS(seenAt)}` : '',
      schedulerId: String((o.caller && o.caller.id) || ''),
    }
    pushLedger(slot, {
      ts: makeTS(), from: '(主会话)', to: '(需求确认)',
      summary: `需求最终稿 ${doc.rel} 指纹 ${doc.digest.slice(0, 12)}`,
      status: '✅ 已登记',
      note: `by=${base.by}${base.note ? ` 自述:${base.note}` : ''};证据=${fresh ? `ask_user_question@${makeTS(seenAt)}` : '未观测到向用户提问(自证)'}`,
    })
    syncSlot(slot, o.coord)
    return Object.assign(base, {
      op: 'registered', at: slot.requirement.at,
      evidence: slot.requirement.evidence,
      evidenceMissing: !fresh,
      note: `已登记:用户对 ${doc.rel} 的确认钉在指纹 ${doc.digest.slice(0, 12)}… 上(该文档一改即自动作废)。`
        + (fresh
          ? `证据:本会话 ${slot.requirement.evidence} 调用过 ask_user_question。`
          : `⚠️ 但本进程**没有观测到**本会话近 ${Math.round(APPROVAL_EVIDENCE_MS / 60000)} 分钟内调用 ask_user_question —— 这条登记是自证的,请人工核对。`),
      nextActions: [
        '生产角色现在可以入场了:relay_spawn role=<角色>,或再来一次 relay action=kickoff',
        '需求文档后续若被改动,指纹漂移会让这条确认**自动作废** —— 改动后请重新登记',
      ],
    })
  }
  ...
  const relayTool = {
    ...
    output: {
      ...
      render(args, value) {
        ...
        /**
         * 需求确认门的状态。与 kickoff 开工回执里那一行**同源**(同一个 requirementStateText),
         * 这里只在"不是开工回执"时单独打印,免得同一句话在一次回执里出现两遍。
         */
        if (v.requirementView && !Array.isArray(v.spawned) && !Array.isArray(v.failed)) lines.push(String(v.requirementView))
        /**
        ...
        if (Array.isArray(v.spawned) || Array.isArray(v.failed)) {
          ...
          // 需求确认门的状态**顶在开工回执最前面**:它决定这一轮谁来了、谁没来
          if (v.requirementView) lines.push(`  ${v.requirementView}`)
          ...
          /**
           * 第四档:被需求确认门跳过的角色。与 failed 分开报是刻意的 ——
           * 混在一起,读的人会去修一个根本没出错的东西;而"生产角色为什么没来"
           * 又必须在第一屏看得见。
           */
          if (Array.isArray(v.deferred)) {
            lines.push(`  deferred(${v.deferred.length}):`)
            for (const d of v.deferred) lines.push(`    ⏳ @${d.role}(${d.label}) 等需求确认 —— ${d.note}`)
            if (v.deferred.length === 0) lines.push('    (无)')
          }
    ...
    async execute(args, exec) {
      ...
      if (action === 'status') {
        ...
        return {
          ...
          // 需求确认门的当前状态(与 workflow_state_status / kickoff 回执同源文案)
          requirementView: requirementStateText(requirementInfoOf(profile, root, slot)),
          /**
  ...
  const spawnTool = {
    ...
    output: {
      ...
      render(args, value) {
        ...
        // 门的两种出格情形各自说清;其余情形只报一行当前状态(同源文案)
        if (v.requirementBypass) lines.push(`⚠️ 绕过需求确认门:${v.requirementBypass}(已记台账一笔「未确认即派活」)`)
        else if (v.requirementView) lines.push(String(v.requirementView))
        if (v.note) lines.push(String(v.note))
        // 拦下时必须给**照做得到**的出路 —— 只报"未派活"就是一条死路
        if (Array.isArray(v.nextActions) && v.nextActions.length > 0) {
          lines.push('出路:')
          for (const n of v.nextActions) lines.push(`  - ${n}`)
        }
  ...
  async function spawnRole(opts) {
    ...
    /**
     * ── 需求确认门(① 上锁)───────────────────────────────────────────────
     * 位置刻意选在"已登记"与"失效重绑"之后:角色**还在场**就照旧走 exists 分支(不拦),
     * 绑定刚被判失效、正要重派的这种,靠台账判断"它入场过"(同样不拦)——
     * 于是门只拦"生产角色**首次**入场",而不动任何一条既有补救路径。
     */
    const reqGate = requirementEntryDecision(profile, root, slot, role, {
      bulk: opts.bulk === true,
      caller: exec && exec.agent,
    })
    if (reqGate.blocked) {
      const callerIsRoot = isRuntimeRoot(exec && exec.agent)
      return {
        status: 'requirement_pending', role, roleLabel: roleDef.label, profileId: profile.id,
        requirement: reqGate.info,
        requirementView: requirementStateText(reqGate.info),
        note: `@${role} 受需求确认门约束,而本项目**还没有**用户对需求最终稿的确认登记(${requirementRelOf(profile)})—— 本次没有派活(kickoff 会跳过它,不是失败)。`
          + (callerIsRoot
            ? ''
            : '注意**你打不开这道门**:角色子会话问不到用户 —— DSH 的 ask_user_question 对"归属于另一个 agent 的子级"直接抛 '
              + 'DELEGATED_CALLER(它要求子会话把未决问题写进最终结果)。钥匙只在**主会话**(人所在的那个会话)手里:'
              + '把需求最终稿交给它,由它向用户确认后登记,或由用户在 ' + ACTIVE_REL + ' 里写一行 approval=skip。'),
        nextActions: requirementEscapeActions(profile),
      }
    }
    /**
     * 主会话亲手派一个"未确认"的生产角色 = 绕过这道门。**不禁止,但绝不静默**:
     * 回执带一行、台账记一笔。理由与"confirmedBy 只标注不拒绝"同源 ——
     * 门挡的是"流程自动往前跑",不是"人想立刻开工";而绕过这件事本身必须可审计。
     */
    let requirementBypass = ''
    if (reqGate.bypass === true) {
      requirementBypass = requirementStateText(reqGate.info)
      pushLedger(slot, {
        ts: makeTS(), from: '(主会话)', to: role, summary: '(绕过需求确认门)',
        status: '⚠️ 未确认即派活',
        note: `由调度者亲手派出 @${role};门状态=${reqGate.info.state};登记对象=${requirementRelOf(profile)}`,
      })
      syncSlot(slot, coord)
      saveStore()
    }
    ...
    return {
      ...
      staleRebind,
      // 门的当前状态**每次都带** —— 包括"这次没被拦"的情形(已登记 / 已入场 / 门没锁),
      // 否则"这个项目到底确认没有"只在他被拦的那一次才看得见。
      requirementView: requirementStateText(reqGate.info),
      ...(requirementBypass ? { requirementBypass } : {}),
  ...
  async function kickoff(opts) {
    ...
    const skipped = []
    const failed = []
    /** 被需求确认门跳过的角色 —— 与 failed 分开报:它不是失败,是"还没到入场的时候"。 */
    const deferred = []
    for (const role of wanted) {
      ...
      // `bulk` 是**调用形态**的开关(开工批量 vs 单点派活),不是角色参数 —— 必须走顶层 opts:
      // 塞进 `a` 里的话 spawnRole 读的是 `opts.bulk`,门就永远拦不住 kickoff(实测踩过)。
      const r = await spawnRole({ a: perRole, exec, root, role, prompt, bulk: true })
      ...
      else if (r.status === 'requirement_pending') deferred.push({ role, label, status: r.status, note: String(r.note || '') })
    ...
    const out = {
      ...
      /**
       * 被需求确认门跳过的角色。**单独一档**、不是 failed:
       * 它没有出错,只是"还没到入场的时候";混进 failed 会让人去修一个不存在的问题。
       */
      deferred,
      requirementView: requirementStateText(requirementInfoOf(profile, root, slotFor(profile, root, coordinators, exec && exec.agent).slot)),
      ...(deferred.length > 0 ? { nextActions: requirementEscapeActions(profile) } : {}),
    ...
    out.note = (coordWarn ? `${coordWarn} ` : '') + (spawned.length > 0
      ? `已派出 ${spawned.length} 个角色${skipped.length > 0 ? `(跳过已登记的 ${skipped.length} 个)` : ''}${activated && !wasActive ? ';本项目已自动激活(不需要 .active 文件)' : ''}`
        ...
        + (deferred.length > 0
          ? `;**需求确认门拦下 ${deferred.length} 个生产角色(${deferred.map((d) => `@${d.role}`).join(' ')}):用户确认需求最终稿之前不派它们**(出路见 nextActions)`
          : '')
      : (skipped.length > 0 ? '所有角色都已登记,未重复派出(要重派传 force=true)' : '一个角色都没派出去,看 failed 里的原因'))
    // 全部被门拦下 = 这一轮其实没开工(没人能给角色派活),必须与"派失败"分开说清
    if (spawned.length === 0 && skipped.length === 0 && deferred.length > 0) {
      out.note = `${out.note}(注意:这一轮**没有任何角色入场** —— 连 ${profile.coordinator} 也被门拦下了;先登记确认,或按 nextActions 放行)`
    }
    return out
  }
  ...
  const STATE_OUTPUT = {
    ...
    render(args, value) {
      ...
      // 需求确认门的状态:与 relay / kickoff / spawn 同源一行(它就是"生产角色能不能入场"的答案)
      if (v.requirementView) lines.push(String(v.requirementView))
      sectionOf('需求确认小节', v.requirement)
      ...
      /**
       * 需求确认的登记结果。被拒的两条路径(`delegated_caller` / `doc_missing`)必须把
       * **为什么拒 + 怎么才对**读进上下文 —— 它们只在 payload 里的话,调用者只会看到
       * "没生效"然后重试同一条路。
       */
      if (v.requirementRegistration && typeof v.requirementRegistration === 'object') {
        const rr = v.requirementRegistration
        lines.push(`【需求确认登记】${rr.op === 'registered' ? '✅ 已登记' : `❌ 被拒(${rr.reason || '未知'})`}`
          + `${rr.docPath ? ` | 对象 ${rr.docPath}${rr.digest ? ` 指纹 ${String(rr.digest).slice(0, 12)}…` : ''}` : ''}`
          + `${rr.by ? ` | by=${rr.by}` : ''}`)
        if (rr.note) lines.push(`  ${rr.note}`)
        if (Array.isArray(rr.nextActions)) for (const n of rr.nextActions) lines.push(`  → ${n}`)
      }
  ...
  const stateTools = [
      ...
      async execute(args, exec) {
        ...
        if (!isActiveGate(gate)) {
          ...
          return {
            ...
            presetAuto: presetAutoPayload(),
            // 门的答案与激活态无关:未激活的项目里,生产角色同样要等确认(而且 kickoff 会当场体现)
            requirementView: requirementStateText(requirementInfoOf(profile, root, slot)),
            note: gate === 'off'
    ...
    },
    {
      ...
      async execute(args, exec) {
        ...
        const s = summarize(cur.text)
        return { op: 'load', status: 'loaded', root, gate: 'active', targetPath: rel, profileId: profile.id, title: s.title, progress: s.progress, requirement: s.requirement, outputs: s.outputs, apiSpecs: s.apiSpecs, todos: s.todos, todoOpen: s.todoOpen, contractLog: s.contractLog, risks: s.risks, summary: cur.text }
      },
    },
    {
       ...
       */
      description: '渲染新的流程状态.md 全文并回传(BEGIN/END 之间),由主会话用 write 工具落盘(插件无工作区写权限)。合并口径:role/nextStep=改「当前进度」小节;outputs/apiSpecs/todos/risks=**整段覆盖**对应小节(传了就以你给的为准,不传才保留磁盘原文);contractRevision=在「契约修订台账」**追加一行**;requirementApproval=**主会话**登记「用户已确认需求最终稿」(钉住需求文档指纹,文档一改即自动作废)。',
      parameters: {
        type: 'object',
        properties: {
          ...ROOT_PROP,
          ...
          requirementApproval: { type: 'object', additionalProperties: true, description: '**只有主会话(live runtime root)能调**:登记「用户已确认需求最终稿」({by:"user", note:"<用户原话摘要>"})。登记把当前需求文档(默认 docs/workflow/项目经理.md,档案可用 requirementFile 覆盖)的 SHA256 钉住 —— 该文档一改即自动作废,要重新确认。角色子会话调用会被拒(它问不到用户,DELEGATED_CALLER)' },
      ...
      async execute(args, exec) {
        ...
        /**
         * 需求确认的登记(② 记账)走**同一个入口**:批准是"改动流程门"的事实,
         * 与状态文档的落盘同源落进插件状态 + 台账,于是它天然有 fingerprint、有时间、有登记人。
         */
        const requirementRegistration = (a.requirementApproval !== undefined && a.requirementApproval !== null && a.requirementApproval !== false)
          ? registerRequirementApproval({
            profile, root, slot, coord, caller, isChild: isChildCaller,
            req: typeof a.requirementApproval === 'object' ? a.requirementApproval : {},
          })
          : null
        // 门状态在**登记之后**再算一次:同一份回执里"刚登记完"就该显示已登记(而不是上一秒的未登记)
        const requirementInfo = requirementInfoOf(profile, root, slot)
        ...
        const documentText = buildDocument(currentText, Object.assign({}, a, {
          header: { coordinator: profile.coordinator },
          // 「需求确认」小节整段由门视图现算 —— 落盘的就是**登记时的指纹**,
          // 人翻文档时能直接对着它看出"这份需求改过之后上一次确认已经不算数"。
        ...
        // (caller / isChildCaller 已在上面按同一判据算好,这里不再重复声明。)
        ...
        return {
          ...
          ledgerPending: (slot.ledger || []).length,
          // 门的当前状态(与 status / relay / kickoff 同源一行)
          requirementView: requirementStateText(requirementInfo),
          // 本次这一笔登记到底成了没有 —— 被拒的两条路径(delegated_caller / doc_missing)必须带着出路回执
          ...(requirementRegistration ? { requirementRegistration } : {}),
  ...
  ctx.on('tools/post-execute', async (exec, result, next) => {
    try {
      ...
      /**
       * 需求确认那道门唯一的**外部事实**:主会话真的调用过 ask_user_question。
       * 只记 live runtime root 的调用(角色子会话本来就调不动那个工具 —— DELEGATED_CALLER);
       * 上限 64 个会话是为了让这张表永远不会长成内存泄漏(它是进程内证据,不落盘)。
       */
      if (toolName === 'ask_user_question' && exec && exec.agent && isRuntimeRoot(exec.agent)) {
        askUserSeen.set(askEvidenceKey(exec.agent.id, cwdOf(exec.agent)), Date.now())
        if (askUserSeen.size > 64) askUserSeen.delete(askUserSeen.keys().next().value)
      }
  ...
  ctx.logger?.info?.(
    ...
    // 需求确认门的档位进 ready 行:"这一版带不带门、带的是哪一档"必须启动期就能自证
    + ` | 需求门=${REQ_MODE.mode}${REQ_MODE.known ? '' : `(配置值 "${REQ_MODE.raw}" 未识别,按 enforce)`}`
