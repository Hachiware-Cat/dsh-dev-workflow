...
name: spike
description: >
  一次性原型验证:用一个丢弃实验回答「这方案行不行」,验证完就丢,不污染正式代码。
whenToUse: >
  需求不确定性高、技术选型没底、动手前想先探路时。
version: 0.0.1
---

# Spike

> **随包说明**:本技能随 `dsh-plugin-dev-workflow` 一起发布(包内 `skills/spike/`),
...
- `pwsh("pip show websockets | grep Version")` — check what's installed in the project's venv
...
```
pwsh("mkdir -p spikes/001-websocket-streaming")
write("spikes/001-websocket-streaming/README.md", "# 001: websocket-streaming\n\n...")
write("spikes/001-websocket-streaming/main.py", "...")
pwsh("cd spikes/001-websocket-streaming && python3 main.py")
# Observe output, iterate.
```

**Parallel comparison spikes (002a / 002b) — delegate.** When two approaches can run in parallel and both need real engineering (not 10-line prototypes), fan out with `subagent`:
