...
name: impeccable
description: >
  前端界面打磨:形状、排版、配色、动效与层次,直到可交付。
whenToUse: >
  界面已能跑,但要提升质感、可读性或一致性时。
version: 0.0.1
---

> **随包说明**:本技能随 `dsh-plugin-dev-workflow` 一起发布(包内 `skills/impeccable/`),
...
1. Run `node <技能目录>/scripts/context.mjs` once per session (if the runtime shows this skill's loaded base directory, run `node <skill-base-dir>/scripts/context.mjs`; keep cwd at the user's project). Pass a named source file or route as `--target <path>`. It loads PRODUCT.md, DESIGN.md, the matching surface brief, and native-platform guidance when applicable; follow its directives and do not rerun it.
