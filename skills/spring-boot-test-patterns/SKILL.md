...
description: >
  Spring Boot 测试范式:单元、切片、集成与 Testcontainers,含 CI 配置与性能取舍。
whenToUse: >
  给 Java 后端写测试,或质检要评审测试覆盖与写法时。
version: 0.0.1
---

# Spring Boot Testing Patterns

> **随包说明**:本技能随 `dsh-plugin-dev-workflow` 一起发布(包内 `skills/spring-boot-test-patterns/`),
