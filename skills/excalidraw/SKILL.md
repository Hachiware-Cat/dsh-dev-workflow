...
name: excalidraw
description: >
  手绘风图表:直接产出可编辑的 Excalidraw JSON(架构/流程/时序)。
whenToUse: >
  要一张可编辑、可分享的手绘风图时。
version: 0.0.1
---
...
> **随包说明**:本技能随 `dsh-plugin-dev-workflow` 一起发布(包内 `skills/excalidraw/`),
...
Wrap your elements array in the standard `.excalidraw` envelope and 用 `write` 保存:
